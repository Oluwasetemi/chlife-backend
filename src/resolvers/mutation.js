...
const mutation = {
    ...
    async adminOnBoardCompany(_, args, { req }) {
        try {
            ...
            // check if user exist on the platform before
            const userExists = await findOneByEmail(
                args.input.organizationEmail
            );
            ...
            const randomBytesPromisified = promisify(randomBytes);
            const resetPasswordToken = (await randomBytesPromisified(
                20
            )).toString('hex');
    ...
    async loginWithUserType(_, args) {
        try {
            ...
            if (!userExist) {
                throw new Error(
                    `User(${args.accountType || ''}) with email not found`
                );
            }
        ...
        try {
            ...
            if (!req.user.currentHra) {
                ...
                const percentageProgress = Math.round(
                    (Number(Object.keys(responseToBeSubmitted).length) / 111) *
                        100
                );
            ...
            if (req.user.currentHra && input.stage !== 'SUBMIT') {
                const hraData = await hra.findById(
                    req.user.currentHra.toString()
                );
            ...
            if (input.stage === 'SUBMIT') {
                ...
                if (!hraData) {
                    throw new Error(
                        'You cannot submit without starting assessment'
                    );
                }
    ...
    async addEmployeeToACompany(_, { input }, { req }) {
        try {
            ...
            return {
                message: `${
                    input.length
                } employee has been added to your company`,
            };
        } catch (error) {
            if (error.code === 11000) {
                throw new Error(
                    `Cannot create user with ${
