...
const mutation = {
    ...
    try {
      ...
      if (!req.user.currentHra) {
        ...
        const percentageProgress = Math.round(
          (Number(Object.keys(responseToBeSubmitted).length) / 155) * 100
        );
      ...
      if (input.stage === 'SUBMIT') {
        ...
        Object.keys(jsonObject).forEach((item) => {
          if (jsonObject[item] === 'not answered') {
  ...
  async ContactUs(_, args) {
    // validate firstName, lastName, CompanyName, Email Address, Phone, Message
    // validate the input that graphql will not validate
    ...
    const data = {
      ...
      lastName: sanitize(args.lastName),
      companyName: sanitize(args.companyName),
